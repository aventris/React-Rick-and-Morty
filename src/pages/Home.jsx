...
const Home = () => {
  ...
  React.useEffect(async () => {
    try {
      ...
      let res = await fetch(`https://rickandmortyapi.com/api/character${query.search}`);
      if (!res.ok)
    ...
    <div className='home'>
      ...
      {!state.loading && !state.error &&
        <>
          ...
          <Searcher input={state.input} handleInput={handleInput} handleSearch={handleSearch}>
            <Filter gender={state.gender} status={state.status} handleGender={handleGender} handleStatus={handleStatus} />
          </Searcher>
          <CharacterList prevPage={0} nextPage={1} pagination={getPagination()}>
            {state.characters.results.map(character =>
              <CharacterItem key={`character-${character.id}`} character={character} />
            )}
          </CharacterList>
        </>
      }
